...
class SlackServiceSpec
  ...
  "ensureChannelExistsAndSyncMembers" should:
    ...
    "create a Slack channel if one does not exist, invite all found users, and update UMP" in new SlackServiceSetup:
      val team = Team(
        teamName          = "PlatOps",
      ...
      service.ensureChannelExistsAndSyncMembers(Seq(team)).futureValue
    ...
    "use existing channel if found, and only add missing members" in new SlackServiceSetup:
      val team = Team(
        teamName          = "PlatOps",
        description       = None,
        documentation     = None,
    ...
    "log and recover when Slack channel creation fails" in new SlackServiceSetup:
      ...
      noException shouldBe thrownBy(service.ensureChannelExistsAndSyncMembers(Seq(team)).futureValue)

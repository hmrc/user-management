  ...
  "getUserRoles" when :
    "parsing a valid response" should :
      "return all roles if assigned" in new Setup:
        ...
        val res: Seq[UserRole] =
          userManagementConnector.getUserRoles("joe.bloggs").futureValue

        res shouldBe(Seq(TeamAdmin, TrustedAuthoriser, LocationAuthoriser, GlobalAuthoriser, ExperimentalFeatures))
      ...
      "return an empty sequence if no roles are assigned" in new Setup:
        ...
        res shouldBe (Seq.empty[UserRole])
    ...
    "it receives a non 2xx status code response" should :
      "return an UpstreamErrorResponse" in new Setup:
        ...
        val res: Throwable =
          userManagementConnector.editUserRoles(editUserRoles).failed.futureValue
  ...
  "editUserRoles" when :
    "parsing a valid response" should :
      "pass the correct payload and return unit" in new Setup:
        ...
        val res: Unit =
          userManagementConnector.editUserRoles(editUserRoles).futureValue
...
trait Setup:
  ...
  val editUserRoles: EditUserRoles =
    EditUserRoles(
      username = "joe.bloggs",

...
object User:
  val format: OFormat[User] =
    ...
    ~ ( __ \ "tools"         ).formatWithDefault[UserAccess](UserAccess(false, false, false, false, false, false))(UserAccess.mongoFormat) // Default only required until data is backfilled in mongo then can be removed
